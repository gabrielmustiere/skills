...
3. **Détecte le stack** : lis `${CLAUDE_SKILL_DIR}/../skills/feature/SKILL.md` n'est pas nécessaire ici — applique la procédure documentée dans `plugins/workflow/references/stacks/_detection.md` (chemin relatif depuis la racine du projet de l'agent : utilise `Glob` pour localiser `references/stacks/_detection.md` si tu n'es pas sûr du chemin). Lis aussi `CLAUDE.md` du projet pour les commandes QA exactes (préfixes `docker compose exec`, `make`, `vendor/bin`, etc.).
